...
fn rotate_camera(
	...
	mut transforms: Query<(&CharacterControllerCameraOf, &mut Transform)>,
) {
	let Ok(camera) = cameras.get(rotate.context) else {
		return;
	};
	let Ok((settings, mut transform)) = transforms.get_mut(camera.get()) else {
		return;
	};
	let (mut yaw, mut pitch, _) = transform.rotation.to_euler(EulerRot::YXZ);

	let delta = -rotate.value;
	yaw += delta.x.to_radians() * settings.mult.x;
	pitch += delta.y.to_radians() * settings.mult.y;
